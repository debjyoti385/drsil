<?php
require_once 'ctShortcodeDecorator.class.php';
...
class ctShortcodeHandler {
	...
	public static function getGroups() {
		return array_unique(array_values(self::$groups));
	}
	...
	public function menu() {
		...
		foreach (self::getGroups() as $group) {
			...
			foreach ($shortcodes[$group] as $shortcode) {
				$node['sub'][] = $shortcode->getShortcodeMenuItem();
			}
			$result[] = $node;
		}
	...
	public function preview() {
		...
		require dirname(__FILE__) . '/view/preview.php';
		exit;
	}
	...
	/*function register_rich_buttons_fullscreen($buttons) {
		$buttons[] = 'separator';
		$buttons['shortcode'] = array(
			'title' => __('shortcodeGenerator', 'ct_theme'),
			'both' => true
		);
		return $buttons;
	}*/
	...
	function add_rich_plugins($plugin_array) {
		$plugin_array['ctShortcode'] = CT_SHORTCODE_TINYMCE_URI . '/plugin.js';
			...
			(function ($) {
				$(document).ready(function () {
					if (typeof shortcodeMenu != 'undefined') {
						shortcodeMenu.init({
							langs: {
								shortcodes: "<?php _e('shortcodes', 'ct_theme');?>",
							}
						});
					}
				});
