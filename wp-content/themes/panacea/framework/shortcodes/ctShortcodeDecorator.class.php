<?php
require_once dirname(__FILE__) . '../../form/utils_formbuilder.inc.php';
...
class ctShortcodeDecorator {
	...
	protected function icon($name, $data) {
		...
		if (!isset($data['help'])) {
			$data['help'] = sprintf(__("View %s and enter icon name ex. glass", 'ct_theme'), '<a target="_blank" id="open_' . $name . '" href="' . $data['link'] . '">' . __('available icons', 'ct_theme') . '</a>');
		}
