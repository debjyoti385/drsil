...
function ct_demo_admin_init() {
	/** @var ctNHP_Options $NHP_OPtions */
	global $NHP_Options;
	add_action('nhp-opts-load-page-' . $NHP_Options->getOptionsPageName(), array('ctImportDemoHandler', 'optionsPage'));
	add_action('wp_ajax_ct-demo-import', array('ctImportDemoHandler', 'handleCtDemoImportAction'));
	add_action('wp_ajax_ct-demo-export', array('ctImportDemoHandler', 'handleCtDemoExportAction'));
	add_action('wp_ajax_ct-demo-rebuild', array('ctImportDemoHandler', 'handleCtDemoRebuildAction'));
}

add_action('admin_init', 'ct_demo_admin_init', 11);

add_action('ct_theme_loader.options.load', array('ctImportDemoHandler', 'addOptions'));

require_once dirname(__FILE__) . '/ctRevolutionSliderImporter.class.php';
require_once dirname(__FILE__) . '/ctLayerSliderImporter.class.php';
add_action('ct_import_demo.recommends', array('ctRevolutionSliderImporter', 'addRecommendation'));
add_action('ct_import_demo.recommends', array('ctLayerSliderImporter', 'addRecommendation'));

if (!class_exists('ctImportDemoHandler')) {

	/**
	 * Class ctImportHandler
	 * 1 click demo import
	 */
	class ctImportDemoHandler {
		...
		public static function addOptions($sections) {
			...
			$section = array(
					'icon' => NHP_OPTIONS_URL . 'img/glyphicons/glyphicons_181_download_alt.png',
					'title' => __('1-Click Demo Import', 'ct_theme'),
		...
		public static function optionsPage() {
			wp_enqueue_script('ct_import_demo', CT_THEME_ADMIN_ASSETS_URI . '/js/import.js', array('jquery'));
		}
		...
		public static function handleCtDemoExportAction() {
			require_once dirname(__FILE__) . '/ctDemoExporterImporter.class.php';
			$import = new ctDemoExporterImporter();
			$import->export();

			exit;
		}

		public static function handleCtDemoRebuildAction() {
			require_once dirname(__FILE__) . '/ctDemoExporterImporter.class.php';
			$import = new ctDemoExporterImporter();
			global $NHP_Options;

			$import->rebuild(new ctImport(), $NHP_Options);
			echo "OK";
			exiit;
		}
	}
...
if (class_exists('WP_Importer')) {
	class ctImport extends WP_Importer {
		...
		/**
		 * Returns dir where we can find data to import
		 * @return string
		 */

		public static function getDemoContentBaseDir() {
			$dir = CT_THEME_DEMO_DIR;

			if (isset($_POST['dir']) && $_POST['dir']) {
				$dir = $dir . DIRECTORY_SEPARATOR . sanitize_file_name($_POST['dir']);
			}
			return $dir;
		}
		...
		public static function getXmlPath() {
			return self::getDemoContentBaseDir() . '/wordpress.xml';
		}
		...
		public static function getThemeOptionsPath() {
			return self::getDemoContentBaseDir() . '/theme_options.txt';
		}
		...
		function dispatch() {
			...
			$baseDir = self::getDemoContentBaseDir();

			//import theme options
			$options = self::getThemeOptionsPath();
			if (file_exists($options)) {
				global $NHP_Options;
				$NHP_Options->import(file_get_contents($options),$this->processed_posts);
			}

			//import WP settings
			require_once dirname(__FILE__) . '/ctDemoExporterImporter.class.php';
			$import = new ctDemoExporterImporter();
			$import->import($this->processed_terms);

			//import revolution slider
			require_once dirname(__FILE__) . '/ctRevolutionSliderImporter.class.php';
			$import = new ctRevolutionSliderImporter();
			$import->import();

			//import layer slider
			require_once dirname(__FILE__) . '/ctLayerSliderImporter.class.php';
			$import = new ctLayerSliderImporter();
			$import->import();


			do_action('ct_import_post');
		 ...
		 */
		function import_WXR() {
			$source = self::getXmlPath();

			if (is_file($source)) {
				$upload_dir = wp_upload_dir();
				$dest = $upload_dir['path'] . '/' . basename($source) . '_.txt';
				copy($source, $dest);
				...
				// Save the data
				return wp_insert_attachment($object, $dest);
			}
		}
