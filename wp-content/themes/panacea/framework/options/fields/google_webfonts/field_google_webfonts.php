...
class NHP_Options_google_webfonts extends NHP_Options{	
	...
	*/
	function __construct($field = array(), $value ='', $parent){
		...
		if(!is_array(json_decode($fonts))){
			...
			if(!$fonts){
				$fonts = @file_get_contents(dirname(__FILE__).'/webfonts.json');
			}
	...
	*/
	function render(){
		...
		echo '</select>';
			echo '<p class="description" style="color:red;">'.__(sprintf('The fonts provided below are free to use custom fonts from the <a href="%1$s" target="_blank">Google Web Fonts directory</a>.<br/>Please <a href="%1$s" target="_blank">browse the directory</a> to preview a font, then select your choice below.',
